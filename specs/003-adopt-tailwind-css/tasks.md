...
- [ ] T015 [US2] Add the `box-border` utility to every element that relied on the still-present universal `* { box-sizing: border-box; }` rule (select/inputs, table `th`/`td.fret-cell`, the string-note input, the result button, the clear button — see the comment in `src/tailwind.css`), then delete that universal rule. Confirm `src/tailwind.css` now contains only the theme-token blocks from T002 (the old `:root`/dark-mode custom-property block and every other rule moved in T003 should already be gone via T004–T014's deletions — remove any stragglers found). Run `npm run build:css` and `npm test` (confirms the unrelated existing test suite still passes). Perform the full manual walkthrough in `quickstart.md` (checks 1–5: light mode, dark mode, touch targets, a new utility-only test element, and narrow-screen sticky behavior).
