...
- [X] T001 Add `tailwindcss` and `@tailwindcss/cli` as devDependencies in `package.json`; add `"build:css": "tailwindcss -i ./src/tailwind.css -o ./src/styles.css"` and `"watch:css": "npm run build:css -- --watch"` scripts. (Deviation from the original plan applied during implementation: leave `"start"` unchanged here — flipping it to run `build:css` first would break `npm start` until `src/tailwind.css` exists, breaking this PR's atomicity. That one-line `start` change is deferred to T003, the first task where the build actually succeeds.) Do not run the build yet (`src/tailwind.css` doesn't exist). Run `npm install` and commit the resulting lockfile.
...
- [X] T003 Move the entire current contents of `src/styles.css` (every existing rule, verbatim, unmodified) to below the `@import`/`@theme` block added in T002 inside `src/tailwind.css`. Run `npm run build:css` to generate `src/styles.css` (Tailwind's own utility layer is included but unused so far, contributing no visible output) and prepend it with a `/* GENERATED by \`npm run build:css\` from src/tailwind.css — do not edit directly */` header comment. Change `"start"` in `package.json` to `"npm run build:css && node scripts/serve.js"` (deferred here from T001 — see T001's note — since this is the first point where the build reliably succeeds). Commit the compiled `src/styles.css`. This is the pivot point: styling authorship has moved from a hand-written `src/styles.css` to `src/tailwind.css`, with zero visual change, and every rule is now individually deletable/replaceable in later tasks.
...
- [X] T004 [US1] Apply utility classes to `<body>` in `src/index.html` reproducing the `body` rule (margin, padding, font stack, `bg-app-bg`, `text-app-fg`); delete that rule from `src/tailwind.css`. (Deviation applied during implementation: the `*` box-sizing rule is NOT deleted here, despite this task's original wording — it's still load-bearing for every not-yet-converted element below that combines padding/border with a width/height via its own still-present legacy rule; deleting it now would change their rendering early, breaking this PR's atomicity. `<body>` itself never needed it, since it has no width/height constraint. See the comment left in `src/tailwind.css` — the plan is to add `box-border` to each element that needs it as it's converted in T007/T010/T011/T013/T014, then delete this rule in T015 once nothing depends on it.) Run `npm run build:css`; verify no visual change.
- [X] T005 [US1] Apply utility classes to the `<header>`'s `<h1>` and each `<section>` in `src/index.html` reproducing the `header h1` and `section` rules; delete those rules from `src/tailwind.css`; rebuild; verify no visual change. (Note found during implementation: there is no `<h2>` anywhere in this app's markup or generated DOM — the `h2` rule was dead CSS with no consumer, so it's deleted outright rather than converted.)
- [X] T006 [US1] Apply utility classes to the `.settings-row` divs and the `.hint` paragraph in `src/index.html` reproducing their current rules (`.hint`'s `0.85rem` font-size used the arbitrary-value utility `text-[0.85rem]`, since it doesn't land on Tailwind's default type scale); delete those rules from `src/tailwind.css`; rebuild; verify no visual change.
- [X] T007 [US1] Apply utility classes directly to `#tuning-preset` and `#capo-input` in `src/index.html` reproducing the shared `select, input[type='number'], input[type='text']` sizing rule; delete that rule from `src/tailwind.css`; rebuild; verify no visual change. (Note: `input[type='text']` has no consumer in this app's markup, same as the `h2` case in T005 — nothing to convert for it. `box-border` is deferred to T015 per the updated plan there — the universal `*` rule still covers this element in the meantime.)
- [X] T008 [US1] Apply utility classes to `#settings-error` in `src/index.html` reproducing the `.error` rule; delete that rule from `src/tailwind.css`; rebuild; verify no visual change.
- [X] T009 [US1] Apply a utility class to `#fretboard-section` in `src/index.html` reproducing its `overflow-x: auto` rule; delete that rule from `src/tailwind.css`; rebuild; verify no visual change.
- [X] T010 [US1] In `src/lib/fretboard.js`, replace the `table.className = 'fretboard'` assignment and the header-cell / `.fret-cell` class assignments with utility classes reproducing the `table.fretboard`, shared `th`/`td.fret-cell` border+sizing rule, and the sticky `th` rule; delete those rules from `src/tailwind.css`; rebuild; verify no visual change (including sticky-header scroll behavior). (The `fretboard` class is dropped entirely from `<table>` — all three legacy rules that referenced it are converted by this task, so nothing else needs it. `<td>` cells keep the literal `fret-cell` class alongside the new utilities, since T012's still-pending `.fret-cell`/`.fret-cell--disabled`/`.fret-cell--selected` rules still match on it.)
- [X] T011 [US1] In `src/lib/fretboard.js`, replace the `.fretboard__string-header`, `.fretboard__string-number`, and `.fretboard__string-note` class assignments with utility classes reproducing their current rules; delete those rules from `src/tailwind.css`; rebuild; verify no visual change.
- [X] T012 [US1] In `src/lib/fretboard.js`, replace the `td.fret-cell`, `.fret-cell--disabled`, and `.fret-cell--selected` class assignments with utility classes reproducing their current rules; delete those rules from `src/tailwind.css`; rebuild; verify no visual change across default/disabled/selected states. (This retires the literal `fret-cell`/`fret-cell--disabled`/`fret-cell--selected` class names entirely — confirmed no longer referenced anywhere in the file.)
- [X] T013 [US1] In `src/lib/fretboard.js`, replace the `td.fretboard__result`, `.fretboard__result-button` (using Tailwind's arbitrary-property syntax `[all:unset]` per research.md for the reset), and `.fretboard__result-cue` class assignments with utility classes, using Tailwind's `hover:`/`focus-visible:` variants to reproduce the existing `:hover`/`:focus-visible` rules; delete those rules from `src/tailwind.css`; rebuild; verify no visual change in resting, hover, and focus states. (Deviation from tasks.md: used the arbitrary-variant form `[&:hover]:`/`[.group:hover_&]:` instead of Tailwind's built-in `hover:`/`group-hover:` for the button's own hover state and its hover-triggered cue opacity — Tailwind's built-in `hover:` wraps in `@media (hover: hover)`, which the original plain `:hover` selector never did, so using it would have silently dropped hover styling on touch-only devices, violating strict parity. `focus-visible:`/`group-focus-visible:` are not media-wrapped, so those built-ins were used as-is.)
- [X] T014 [US1] Apply utility classes to `#clear-button` in `src/index.html` reproducing its current rule; delete that rule from `src/tailwind.css`; rebuild; verify no visual change. (All of T004-T014 are now done. Confirmed while doing this: nothing references the legacy `--bg`/`--fg`/`--accent`/`--border`/`--disabled-bg`/`--disabled-fg`/`--error` custom properties anymore — every rule that used them has been deleted — so T015's cleanup of the old `:root`/dark-mode block is now unblocked. Note for T015: that block's `color-scheme: light dark;` declaration is NOT dead — it has no Tailwind utility equivalent and must be preserved, e.g. as a small standalone retained `:root { color-scheme: light dark; }` rule, when the rest of that block is deleted.)
...
- [X] T015 [US2] Add the `box-border` utility to every element that relied on the still-present universal `* { box-sizing: border-box; }` rule (select/inputs, table `th`/`td.fret-cell`, the string-note input, the result button [already had it from T013], the clear button), then delete that universal rule along with the rest of the now-fully-dead old `:root`/dark-mode custom-property block (`--bg`/`--fg`/`--accent`/`--border`/`--disabled-bg`/`--disabled-fg`/`--error` — confirmed unreferenced anywhere as of T014). Preserved `color-scheme: light dark;` as a small standalone `:root` rule, since it has no Tailwind utility equivalent. `src/tailwind.css` now contains only the `@import`s, `@source`s, the `@theme` token block (light + dark), and that one preserved `:root { color-scheme }` rule — nothing else. Ran `npm run build:css` and `npm test` (18/18 pass, unrelated suite unaffected) and confirmed via `npm start` that `/`, `/styles.css`, `/lib/app.js`, `/lib/fretboard.js` all serve correctly.

  **Manual walkthrough (`quickstart.md` checks 1–5) still needs a human with a browser** — no browser tooling was available in this implementation session. Please run through light mode, dark mode, touch-target sizing, adding a temporary utility-only test element, and narrow-screen sticky behavior before merging this PR.
